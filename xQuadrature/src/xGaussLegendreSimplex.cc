...
{
void brickToTet(double xi, double eta, double zeta, double *r, double *s, double *t, double *J);
...
int GaussLegendreTet(int n1, int n2, int n3, xIntPt3d *pts)
{
   ...
   for (i = 0; i < n1; i++)
   {
      for (j = 0; j < n2; j++)
      {
         for (k = 0; k < n3; k++)
         {
            brickToTet(pt1[i], pt2[j], pt3[k], &(pts[index].pt[0]), &(pts[index].pt[1]), &(pts[index].pt[2]), &dJ);
...
int GaussLegendreTri(int n1, int n2, xIntPt2d *pts)
{
