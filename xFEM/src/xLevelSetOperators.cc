...
void xLambdaModifierOnVertices::visit(xLevelSet &f, xRegion target)
{
   for (auto e : target.range(0))
   {
      mVertex *v = static_cast<mVertex *>(e);
...
void xVelocityInitialization::visit(xLevelSet &velo, xRegion target)
{
   ...
   for (auto e : target.range(0))
   {
      mVertex *ve = static_cast<mVertex *>(e);
      velo(ve) = 0.;
   }

   // see attic for what have been set parallel at this level
   ...
   for (auto e : regloc.range(0))
   {
      ...
      if (target.dim() == 3)
      {
         ...
         for (auto se : fs.range())
         {
            if (se->size(0) == 0)  // se is a vertex
            {
               se->print();
               mVertex *v1 = static_cast<mVertex *>(se);
               velo(v) = velofront(v1);
               return;
            }

            //  ... on va chercher les deux noeuds du segment
            mVertex *v1 = static_cast<mVertex *>(se->get(0, 0));
            mVertex *v2 = static_cast<mVertex *>(se->get(0, 1));
         ...
         mVertex *v3 = static_cast<mVertex *>(mem2->get(0, 0));
         mVertex *v4 = static_cast<mVertex *>(mem2->get(0, 1));
         assert((dis1 >= 0.) && (dis1 <= 1.));
         velo(v) = (dis1 * velofront(v3) + dis2 * velofront(v4)) / (dis1 + dis2);
      }
      else  // cas 2D ... point le + proche
      {
         for (auto e2 : fs.range(0))
         {
            mVertex *v1 = static_cast<mVertex *>(e2);
            xtensor::xPoint p1 = v1->point();
...
void xComputeError::loadReference(const xLevelSet &ref) { reference = ref; }
...
void xL2norm::visit(const xLevelSet &current, xRegion target)
{
   ...
   // this part have been uncomented from a PARALLEL directive and should be checked
   assert(xtool::workInProgress());
   double refnormTot = 0., errnormTot = 0.;
   MPI_Allreduce(&refnorm, &refnormTot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
   MPI_Allreduce(&errnorm, &errnormTot, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);

   errnorm = sqrt(errnormTot);
   refnorm = sqrt(refnormTot);
}

void xFitToVertices::visit(xLevelSet &ls, xRegion target)
{
   // NOTE : it could have been a std::set but here we have the
   // same result without balancing the RBtree ... with expected low memory consumption.
   // To check
   xinterface::aomd::xAttachedDataManagerAOMD<bool> verticesFit;

   // loop over the edges in the target
   for (xIter it = target.begin(1); it != target.end(1); ++it)
   {
      // get the value of the level set at the two end points
      mEntity *e = *it;
      mVertex *v0 = (mVertex *)e->get(0, 0);
      mVertex *v1 = (mVertex *)e->get(0, 1);
      double ls0 = ls(v0) - fit_value;
      double ls1 = ls(v1) - fit_value;
      ...
      if (ls0 * ls1 <= 0.0 && ls0 != 0.0 && ls1 != 0.0)
      {
         ...
         {
            mEntity *nv1 = v1;
            verticesFit.setData(*nv1) = true;
            ls(nv1) = fit_value;
         }
      }
   }

   // create comunication keys for vertices modified
   xFitToVertexKeyManager km(target.getPartitionManager());
   xtool::xKeyContainerSendOrRecv<xFitToVertexKeyManager::information_key_t> kc(target.getPartitionManager().getComm());
   kc.accumulateKeys(verticesFit.beginKey(), verticesFit.endKey(), km);

   // create infomanager
   xFitToVertexInfoManager im(ls, target.getPartitionManager(), fit_value);

   // exchange information and finalize fitting
   xtool::exchangeInformation(kc, im);

   return;
}

xFitToVertices::xFitToVertexKeyManager::xFitToVertexKeyManager(const partmanAOMD_t &partman_) : partman(partman_){};
auto xFitToVertices::xFitToVertexKeyManager::localObjectKey(const AOMD::mEntity &o) -> information_key_t { return &o; }
std::set<int> xFitToVertices::xFitToVertexKeyManager::getMessageRanks(const information_key_t &lk)
{
...
xFitToVertices::xFitToVertexInfoManager::xFitToVertexInfoManager(xLevelSet &ls_, const partmanAOMD_t &partman_, double fit_value_)
    : partman(partman_), ls(ls_), fit_value(fit_value_)
{
}
auto xFitToVertices::xFitToVertexInfoManager::getInfo(information_key_t lk, int sendto) -> information_t
{
   // lk is the pointer to local object
   // So we can used it to access to remote object
   xtool::xConstPartitionObject<AOMD::mEntity> po = partman.getConstPartitionObject(*lk);
...
void xFitToVertices::xFitToVertexInfoManager::setInfo(const std::vector<information_t> &info, int receivedfrom)
{
   for (auto v : info) ls(v) = fit_value;
}

void xLsetHamiltonJacobi::byelemcomputation(const std::vector<double> &lsv, const xtensor::xVector<> &gradlvlset, const double &F,
                                            const double &f, const xElementInfo &info, std::vector<double> &maxalphatild,
                                            double &sommaxalphatild, double &somdeltafi) const
{
...
double xCFL::getDt(xRegion s) { return ((xCFL::getLength(s)) / 1. / 2.); }
...
double xCFL::getLength(xRegion s)
{
   ...
   MPI_Allreduce(MPI_IN_PLACE, &hmin, 1, MPI_DOUBLE, MPI_MIN, s.getPartitionManager().getComm());
...
// {
//    if (e->getType() == mEntity::VERTEX) val = (*ls)(e);
// }
// 
...
void xLevelSetSmoother::visit(const xLevelSet &in, xLevelSet &out)
{
   ...
   for (auto e : in.getSupport().range(0))
   {
      mVertex *v = static_cast<mVertex *>(e);
      xtensor::xPoint P0 = v->point();
      int nedge = v->size(1);
      xlinalg::xDenseMatrix M(3, 3);
      xlinalg::xCSRVector V(3), X(3);
      for (int i = 0; i < 3; ++i)
      {
         V(i) = 0;
         X(i) = 0.;
      ...
      for (int i = 0; i < nedge; ++i)
      {
         ...
         mVertex *vi = static_cast<mVertex *>((edge->get(0, 0) == v) ? edge->get(0, 1) : edge->get(0, 0));
         xtensor::xPoint Pi = vi->point();
         double xi = Pi(0) - P0(0);
         double yi = Pi(1) - P0(1);
         double lsi = in(vi);
         M(0, 0) += xi * xi;
...
double xEvalShiftLevelSetModifier::getMaxShift() { return max_shift; }
void xEvalShiftLevelSetModifier::visit(xLevelSet &ls, xRegion target)
{
   ...
   for (xIter it = target.begin(0); it != target.end(0); ++it)
   {
      mVertex *v = (mVertex *)*it;
      int nb = v->size(dim);
      mEntity *upper = nullptr;
      for (int i = 0; i < nb; ++i)
      {
         upper = v->get(dim, i);
         if (filter_upper(upper))
            break;
         else
            upper = nullptr;
      }
