...
{
  public:
   typedef std::unordered_map<KEY, VAL*, HASH, EQUAL> container_t;
  ...
  public:
   ...
   std::pair<iterator, bool> insert(const value_type& v) { return vals.insert(v); }
...
class xValManager
{
  ...
  public:
   ...
   {
      MapContainer.clear();
      for (iter_subsets it = subsets.begin(); it != subsets.end(); ++it) delete it->second;
   }
   ...
   void insert(const K& key, V* v) { MapContainer.insert(std::make_pair(key, v)); }
   ...
   void PrintForDebug(const std::string& filename)
   {
      ...
      if (debug) std::cout << "taille MapContainer " << MapContainer.size() << std::endl;

      std::ofstream out(filename.c_str());
      PrintForDebug(out);
   }

   size_t size(void) { return MapContainer.size(); }

   template <class S, class Op>
   void getAll(std::vector<std::pair<K, S>>& buf)
   {
  ...
  protected:
   ...
   subset_t* getSubset(const std::string& sub) const
   {
      ...
      if (it == subsets.end())
      {
         if (debug) std::cerr << "The entities set with name " << sub << " does not exist\n";
