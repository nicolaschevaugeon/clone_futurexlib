...
"DISPLACEMENT_R", "DISPLACEMENT_TETA", "DISPLACEMENT_ZC",
"KINEMATIC_X", "KINEMATIC_Y", "KINEMATIC_Z", "CONTACT"
"IMPROVEMENT", "IMPROVEMENT_X", "IMPROVEMENT_Y", "IMPROVEMENT_Z",
"MULTIPLIER", "MULTIPLIER_X", "MULTIPLIER_Y", "MULTIPLIER_Z",
"ANY", "ANY_X", "ANY_Y", "ANY_Z",
...
"FIX", "FIX_AND_MEASURE", "FORMULATION_ONE", "FORMULATION_TWO",
...
"ELAST_TRACTION_X",  "ELAST_TRACTION_Y", "ELAST_TRACTION_Z", 
"ZONE", "ZONE_ENRICH",  "ZONE_ENV", "ZONE_POSTPRO",
"ZONE_NAME",
"REFERENCE_SOLUTION_FILE", "STORE_SOLUTION_FILE", "STRESS", "SING_STRESS", 
...
// DEFAULT CONSTRUCTOR
 xData::xData(void) :mesh(0), PhysicalEnv(0), MaterialManager(xMaterialManagerSingleton::instance()) { 
...
xData::~xData(void) { 
  ...
  // I think the bug is not in AOMD ... In fact it seems the destructor break done when deleling attachedData related to xcut::xPhysSurfByTagging. 
  // if (mesh) delete mesh;
...
char* xData::GetProcedureParametersFile(void)
{
  return procedure_param_file;
}
...
char * xData::GetCrackDefinitionFile(void) 
{ 
  return crack_definition_file;
}
...
void xData::ReadMesh(void)
{
  ...
  //  std::cout << "nproc says AOMD :" <<  AOMD::ParUtil::Instance()->size() <<   std::endl;
  if( AOMD::ParUtil::Instance()->size() == 1 ){
	...
	f.close();
	if( mesh != 0 ){
	  cout << " WARNING : non-null xMesh pointer reinitialized!" << endl;
	  delete(mesh);
	  mesh=0;
	}
	...
	mesh = new xMesh(mesh_file);
      ...
      {
	f.close();
	cerr<< "no mesh file with name "<< mesh_file << "found " << endl;
	throw "no mesh file !";
      }
  }
  else {
    string meshPartitionName;
    meshPartitionName.assign( mesh_file,
			      strlen( mesh_file ) - 4 );
    std::stringstream no;
    no << meshPartitionName
       << "-proc" << AOMD::ParUtil::Instance()->rank() 
       << ".msh"; 
   
    meshPartitionName = no.str();
    cout << " Read mesh : " << meshPartitionName << endl;

    
    if( mesh != 0 ){
      std::cout << "Proc " << AOMD::ParUtil::Instance()->rank() 
      << " WARNING : non-null xMesh pointer reinitialized!" << endl;
      delete(mesh);
      mesh=0;
    }
    ifstream f(meshPartitionName.c_str());
    if (!f.good())
      {
	cout<< "no mesh file with name "<< meshPartitionName << "found " << endl;
	throw "no mesh file !";
      }
    mesh = new xMesh(meshPartitionName);
...
const int CONTACT_LINE_ACTIVE    = 22;
  ...
  while( (i = fscanf(fp, "%[1234567890A-Z_]",  key)) != EOF) {
	...
	if (strcmp(key, "IC_VOLUME") == 0)  {fscanf(fp, "%d%*[\n= {]", &entity); 
		                           InfoEnv = PhysicalEnv;
					   check_active(active, "IC_VOLUME");
					   geom    = IC_VOLUME;
					   active  = IC_VOLUME_ACTIVE;}
	if (strcmp(key, "BC_VOLUME_NAME") == 0)  {fscanf(fp, "%s%*[\n= {]", entity_name); 
					   InfoEnv = PhysicalEnv;
	...
	if (strcmp(key, "IC_SURFACE") == 0)  {fscanf(fp, "%d%*[\n= {]", &entity); 
					   InfoEnv = PhysicalEnv;
	...
	if (strcmp(key, "RIGID_SURFACE") == 0)  {fscanf(fp, "%d%*[\n= {]", &entity); 
					   InfoEnv = PhysicalEnv;
	...
	if (strcmp(key, "BC_SURFACE_NAME") == 0)  {fscanf(fp, "%s%*[\n= {]", entity_name);
					   InfoEnv = PhysicalEnv;
	...
	if (strcmp(key, "IC_POINT") == 0)  {fscanf(fp, "%d%*[\n= {]", &entity); 
		                           InfoEnv = PhysicalEnv;
					   check_active(active, "IC_POINT");
					   geom    = IC_POINT;
					   active  = IC_POINT_ACTIVE;}

	if (strcmp(key, "BC_POINT_NAME") == 0)  {fscanf(fp, "%s%*[\n= {]", entity_name); 
					   InfoEnv = PhysicalEnv;
